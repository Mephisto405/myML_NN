...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    # softmax    
    norm = scores - np.max(scores, axis = 1, keepdims = True) # avoid instability
    norm = np.exp(norm)
    norm /= np.sum(norm, axis = 1, keepdims = True) # normalized
    
    loss = - np.sum(np.log(norm[np.arange(N), y]))
    loss /= N
